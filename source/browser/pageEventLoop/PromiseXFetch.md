...
date: 2024年05月06日17:45:58
---

在[上一篇文章](/browser/pageEventLoop/MacroAndMicroTasks.html)中我们聊到了微任务是如何工作的，并介绍了MutationObserver是如何利用微任务来权衡性能和效率的。今天我们就接着来聊聊微任务的另外一个应用Promise，DOM/BOM API中新加入的API大多数都是建立在Promise上的，而且新的前端框架也使用了大量的Promise。可以这么说，Promise已经成为现代前端的“水”和“电”，很是关键，所以深入学习Promise势在必行。
...
![Web应用的异步编程模型](https://pic.imgdb.cn/item/6638a7140ea9cb14038ca1f1.png)
...
![封装请求过程](https://pic.imgdb.cn/item/6638a79e0ea9cb14038dd694.png)
...
![回调函数返回值穿透到最外层](https://pic.imgdb.cn/item/6638a8b10ea9cb1403902e8d.png)
...
上面采用了定时器来推迟onResolve的执行，不过使用定时器的效率并不是太高，好在我们有微任务，所以Promise又把这个定时器改造成了微任务了，这样既可以让onResolve_延时被调用，又提升了代码的执行效率。这就是Promise中使用微任务的原由了。详细的Promise实现可以参考[这篇文章](/javascript/promise.html)。
